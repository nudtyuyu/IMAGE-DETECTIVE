...
int test_sm2_encrypt(void*data,void *encrypt,int count)
{
	...
	unsigned char *encode  = (unsigned char *)encrypt;
	int msg_len = count;
	...
	SM2_KEY_PAIR*p=&key_pair;
	/*把SM2加密生成的C1，C2，C3部分按标准排列，稍作修改可以支持老版本标准，这里是新版标准C1C3C2的顺序*/
	for (i = 0; i < C1_LENGTH; i++)
	{
		encode[i]=c1[i];
		
	}
	for (i; i < C1_LENGTH+C3_LENGTH; i++)
	{
		encode[i]=c3[x];
		x++;

	}
	for (i; i < C1_LENGTH+C3_LENGTH+msg_len; i++)
	{
		encode[i]=c2[y];
		y++;

	}
	for (i; i < C1_LENGTH+C3_LENGTH+msg_len+sizeof(SM2_KEY_PAIR); i++)
	{
		encode[i]=((unsigned char*)p)[z];
		z++;
	}
...
int test_sm2_decrypt(void*origin,void*secret0,void *decrypt,int count)
{
	int error_code;
	unsigned char*secret=(unsigned char*) secret0;
	unsigned char *msg = (unsigned char *)origin;
	unsigned char *c2;
	unsigned char *decode  = (unsigned char *)decrypt;
	int msg_len = count-C1_LENGTH-C3_LENGTH-sizeof(SM2_KEY_PAIR);
	SM2_KEY_PAIR key_pair;
	SM2_KEY_PAIR*p = &key_pair;
	unsigned char c1[65], c3[32];
