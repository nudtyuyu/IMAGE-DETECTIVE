...
int IsAImageFile(const char* fileName) {
	...
	return -1;
     ...
     //printf("head:%.2lx\n",head);
     //printf("tail:%.2lx\n",tail);
     //printf("result: %.2lx\n",(((head & 0x0000ffff) ^ 0x0000ffd8) | ((tail & 0x0000ffff) ^ 0x0000d9ff)));
     return (((head & 0x0000ffff) ^ 0x0000ffd8) | ((tail & 0x0000ffff) ^ 0x0000d9ff));
 }
...
int main(int argc,char **argv)
{
	...
	//int num = 0;
	filenum=0;
	char* path_in = (char*)malloc(sizeof(char)*PATHNAME);
	char* path_out = (char*)malloc(sizeof(char)*PATHNAME);
	int s;
	for(i=0;i<argc;i++)
	{
	...
	int cpu_num = sysconf(_SC_NPROCESSORS_CONF)-2;
	//int cpu_num =4;
	//printf("cpu:%d\n",cpu_num);
	int pic_num = filenum/cpu_num;
	struct thread_info* info;
	info = calloc(cpu_num,sizeof(struct thread_info));
	if(info==NULL)
		handle_error_en(0,"calloc");
	struct parameter* arg[cpu_num];
	start = clock();
	for(j=0;j<cpu_num;j++)
	{
		arg[j] = (struct parameter*)malloc(sizeof(struct parameter));
		info[j].thread_num=j;
		//printf("thread_num:%d\n",info[j].thread_num);
		arg[j]->t_info = &info[j];
		arg[j]->num = pic_num;
		arg[j]->argc = argc;
		arg[j]->argv = (char**)malloc(sizeof(char*)*argc);
		for(i=0;i<argc;i++)
		{
			arg[j]->argv[i] = (char*)malloc(sizeof(char)*PATHNAME);
			strcpy(arg[j]->argv[i],argv[i]);
		}
		arg[j]->id = j;
		arg[j]->path_in  = (char*)malloc(sizeof(char)*PATHNAME);
		arg[j]->path_out = (char*)malloc(sizeof(char)*PATHNAME);
		strcpy(arg[j]->path_in,path_in);
		strcpy(arg[j]->path_out,path_out);
		s = pthread_create(&info[j].thread_id,NULL,thread_do,arg[j]);
		if(s!=0)
		{
	...
	for (j = 0; j < cpu_num; j++) {
        	s = pthread_join(info[j].thread_id, NULL);
        	if (s != 0) {
            		handle_error_en(s, "pthread_join");
        	}
        end = clock();
        printf("解压缩%d张图片用时: %.6lf s\n",filenum+4,((double)(end-start))/CLOCKS_PER_SEC/cpu_num);
        return 0;
    }
...
void* thread_do(void* arg)
{
	struct parameter* p= arg;
	int s,j,i=0;
	int num = p->num;
	int id = p->id;
	int argc = p->argc;
	char**argv =  p->argv;
	pthread_t thread = p->t_info->thread_id;
	cpu_set_t cpuset;
	CPU_ZERO(&cpuset);
	CPU_SET(p->t_info->thread_num,&cpuset);
	...
	for(j=1+id*num;j<id*num+num+1;j++)
	{
		
		//printf("thread:%d\tj:%d\n",p->id,j);
		char filename_in[256];
		char filename_out[256];
		strcpy(filename_in,p->path_in);
		strcat(filename_in,"/");
		char number[10];
		char number2[10];
		snprintf(number2,10,"%d",k);
		snprintf(number,10,"%d",j);
		strcat(filename_in,number);
		strcat(filename_in,".jpg");
		strcpy(filename_out,p->path_out);
		strcat(filename_out,"/");
		strcat(filename_out,number2);
		strcat(filename_out,".bmp");
		//strcpy(file,filename_in);
		//printf("check:%d\n",IsAImageFile(filename_in));
		if(IsAImageFile(filename_in)==0)
		{
			//printf("yesyesyes\n");
			k++;
		}
		...
		for(i=0;i<argc;i++)
		{
			...
			if(!strcmp(argv[i],"-outfile"))
			{
				...
				strcpy(argv[i+1],filename_out);
			}
			else if(i==argc-1)
			{
				strcpy(argv[i],filename_in);
			}
