    ...
    public:
        ...
        void Compress(char * filename)
        {
			...
			if(j==num_col-1)
                        ...
                        if(i==num_row-1)
                            end_row=0;
                        for(int ii=start_row;ii>start_row-len_line;ii--)
			...
			for(int jj=end_col;jj>end_col-len_line;jj--)
			{
			   for(int ii=start_row;ii>=end_row;ii--)
				r[ii*width+jj]=255,g[ii*width+jj]=b[ii*width+jj]=0;
			}
...
int main()
{
    ...
    clockBegin = clock();
    char bmp_filename[100]="from/display.bmp";
    char bmp2_filename[100]="result.bmp";
    jpeg a=jpeg();//初始化
